...
LVIS_V1_DATASET_CLASSES = ('aerosol_can', 'air_conditioner', 'airplane', 'alarm_clock',
...
class LVISV1Dataset(LVISDataset):
    ...
    METAINFO = {
        'classes': LVIS_V1_DATASET_CLASSES,
        'palette': None
    }
    ...
    def load_data_list(self) -> List[dict]:
        ...
        rare_classes = set()
        rare_labels = set()
        for c in self.lvis.cats.values():
            if c['frequency'] != 'r':
                continue
            rare_classes.add(c['name'])
            cat_id = c['id']
            label_id = self.cat2label[cat_id]
            assert self._metainfo['classes'][label_id] == c['name'], 'sanity'
            rare_labels.add(label_id)

        assert len(rare_classes) == len(rare_labels), 'mismatch in rare sets length'
        self._metainfo['rare_classes'] = rare_classes
        self._metainfo['rare_labels'] = rare_labels
